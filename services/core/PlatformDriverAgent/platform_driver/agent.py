...
from volttron.platform.agent import math_utils, utils
...
def platform_driver_agent(config_path, **kwargs):
    ...
    if config.get("driver_config_list") is not None:
        _log.warning(
            "Platform driver configured with old setting. This is no longer supported."
        )
        _log.warning(
...
class PlatformDriverAgent(Agent):
    def __init__(
        ...
        **kwargs,
        ...
        self.vip.config.subscribe(
            self.update_driver, actions=["NEW", "UPDATE"], pattern="devices/*"
        )
        self.vip.config.subscribe(
            self.remove_driver, actions="DELETE", pattern="devices/*"
        )
    ...
    def configure_main(self, config_name, action, contents):
        ...
        if action == "NEW":
            try:
                ...
                if self.max_open_sockets is not None:
                    ...
                    _log.info(
                        "maximum concurrently open sockets limited to "
                ...
                _log.error(
                    "ERROR PROCESSING STARTUP CRITICAL CONFIGURATION SETTINGS: {}".format(
                        e
                    )
                )
            ...
            try:
                if (
                    self.scalability_test_iterations
                    != int(config["scalability_test_iterations"])
        ...
        if (
            ...
            _log.info(
                "Setting time delta between driver device scrapes to  "
    ...
    def set_multiple_points(self, path, point_names_values, **kwargs):
        ...
        if path in self._override_devices:
            ...
            return self.instances[path].set_multiple_points(
                point_names_values, **kwargs
            )
    ...
    def heart_beat(self):
        ...
        for device in self.instances.values():
            try:
                ...
                _log.warning(
                    f"Failed to set heart_beat point on device: {device.device_name} -- {e}."
                )
    ...
    def revert_point(self, path, point_name, **kwargs):
        ...
        if path in self._override_devices:
            raise OverrideError(
                "Cannot revert point on device {} since global override is set".format(
                    path
                )
            )
        else:
    ...
    @RPC.export
    def set_override_on(
        self, pattern, duration=0.0, failsafe_revert=True, staggered_revert=False
    ):
        """RPC method
    ...
    def _set_override_on(
        ...
        for name in self.instances.keys():
            ...
            if fnmatch.fnmatch(name, pattern):
                ...
                if failsafe_revert:
                    if staggered_revert:
                        self.core.spawn_later(
                            i * stagger_interval, self.instances[name].revert_all()
                        )
                    else:
...
def main(argv=sys.argv):
    ...
    utils.vip_main(platform_driver_agent, identity=PLATFORM_DRIVER, version=__version__)
