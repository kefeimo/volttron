...
class FakeAgent(Agent):
    ...
    def __init__(self, config_path: str, **kwargs) -> None:
        ...
        try:
            ...
            _log.error(e)
            _log.info(
                f"Failed to use config_from_path {config_from_path}"
    ...
    @RPC.export
    def rpc_call_driver(self, VolttronPointName) -> str:
        """
        a proxy funnction to call rpc_dummy
        """
        ...
        rs = self.vip.rpc.call(
            peer, "get_point", "chargepoint/MSL5/port2", VolttronPointName
        ).get(timeout=10)
...
def main():
    ...
    utils.vip_main(FakeAgent, version=__version__)
